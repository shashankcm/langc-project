...
def chroma_basics():
    ...
    print(
        f"Vectorstore created with {vectorstore._collection.count()} documents and persisted to {temp_dir}"
    )
    ...
    for i, result in enumerate(results):
        print(
            f"Result {i + 1}: {result.page_content} (Source: {result.metadata['source']})"
        )
...
def similarity_search_with_scores():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for i, (result, score) in enumerate(results):
            print(
                f"Result {i + 1}: {result.page_content} (Source: {result.metadata['source']}) - Score: {score}"
            )
...
def metadata_filtering():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        results = vectorstore.similarity_search_with_score(
            query, k=3, filter={"topic": "database"}
        )
        print(f"Top 3 results with scores for given query '{query}':")
        for i, (result, score) in enumerate(results):
            print(
